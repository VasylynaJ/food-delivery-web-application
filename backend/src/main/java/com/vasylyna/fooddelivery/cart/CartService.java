...
@Service public class CartService {
 private static final BigDecimal DELIVERY_FEE=new BigDecimal("3.50");
 ...
 private CartView view(Cart c){var rows=items.findByCart_Id(c.getId());var lines=rows.stream().map(x->new CartLine(x.getMenuItem().getId(),x.getMenuItem().getName(),x.getMenuItem().getPrice(),x.getQuantity(),x.getMenuItem().getPrice().multiply(BigDecimal.valueOf(x.getQuantity())))).toList();BigDecimal subtotal=lines.stream().map(CartLine::lineTotal).reduce(BigDecimal.ZERO,BigDecimal::add).setScale(2,RoundingMode.HALF_UP);BigDecimal fee=lines.isEmpty()?BigDecimal.ZERO:DELIVERY_FEE;return new CartView(lines,subtotal,fee,subtotal.add(fee));}
