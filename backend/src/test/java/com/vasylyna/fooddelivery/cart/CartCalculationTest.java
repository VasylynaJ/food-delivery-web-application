...
class CartCalculationTest {
 ...
 @Test void totalIncludesQuantityAndDeliveryFee(){
  ...
  var food=mock(com.vasylyna.fooddelivery.menu.MenuItem.class);when(food.getName()).thenReturn("Noodles");when(food.getPrice()).thenReturn(new BigDecimal("8.25"));var line=mock(CartItem.class);when(line.getMenuItem()).thenReturn(food);when(line.getQuantity()).thenReturn(2);when(items.findByCart_Id(5L)).thenReturn(List.of(line));
  var service=new CartService(carts,items,mock(MenuItemRepository.class),mock(AppUserRepository.class));var view=service.get("a@b.test");assertThat(view.subtotal()).isEqualByComparingTo("16.50");assertThat(view.deliveryFee()).isEqualByComparingTo("3.50");assertThat(view.total()).isEqualByComparingTo("20.00");
 }
}
